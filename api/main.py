...
def _detect_local_lora() -> tuple[str, str] | None:
    candidates = [
        Path("outputs/lora-qwen2.5-0.5b-full"),
        Path("outputs/lora-qwen2.5-0.5b-smoke"),
    ]
...
def _kb_paths() -> dict[str, Path]:
    config = get_runtime_config()
    return {
        "source_dir": _project_path(os.getenv("KB_SOURCE_DIR", "kb_sources")),
        "manifest_path": _project_path(os.getenv("KB_MANIFEST_PATH", "vector_db/source_manifest.json")),
