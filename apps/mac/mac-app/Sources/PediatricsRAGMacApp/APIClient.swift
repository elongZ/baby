...
struct APIClient {
    ...
    func visionStatus() async throws -> VisionStatusResponse {
        let (data, response) = try await URLSession.shared.data(from: baseURL.appending(path: "vision/status"))
        try validate(response: response, data: data)
        return try JSONDecoder().decode(VisionStatusResponse.self, from: data)
    }

    func predictVision(imagePath: String) async throws -> VisionPredictionResponse {
        var request = URLRequest(url: baseURL.appending(path: "vision/predict"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try JSONEncoder().encode(VisionPredictRequest(imagePath: imagePath))
        let (data, response) = try await URLSession.shared.data(for: request)
        try validate(response: response, data: data)
        return try JSONDecoder().decode(VisionPredictionResponse.self, from: data)
    }

    func visionEvaluationSamples() async throws -> VisionEvaluationSamplesResponse {
        let (data, response) = try await URLSession.shared.data(from: baseURL.appending(path: "vision/evaluation/samples"))
        try validate(response: response, data: data)
        return try JSONDecoder().decode(VisionEvaluationSamplesResponse.self, from: data)
    }

    func detectionStatus() async throws -> DetectionStatusResponse {
        let (data, response) = try await URLSession.shared.data(from: baseURL.appending(path: "detection/status"))
        try validate(response: response, data: data)
        return try JSONDecoder().decode(DetectionStatusResponse.self, from: data)
    }

    func predictDetection(
        imagePath: String,
        confidenceThreshold: Double? = nil,
        iouThreshold: Double? = nil
    ) async throws -> DetectionPredictionResponse {
        var request = URLRequest(url: baseURL.appending(path: "detection/predict"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
